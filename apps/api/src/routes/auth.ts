...
export function createAuthRoutes() {
  ...
  return (
    app
      .post(
        ...
        async ({ body, set, jwt }) => {
          try {
            ...
            const shouldLogMagicLink =
              isDev && process.env.LOG_MAGIC_LINKS === "true";
      ...
      .post(
        ...
        async ({ body, set, jwt }) => {
          try {
            ...
            if (!isValid) {
              trackFailure("otp.verification", "Invalid or expired code", {
                email,
              });
              set.status = 401;
            ...
            const refreshExpiryDays =
              parseInt(refreshTokenExpiry.replace("d", ""), 10) || 30;
      ...
      .post(
        ...
        async ({ body, set, jwt }) => {
          try {
            ...
            authLogger.error(
              { err: error, refreshToken: body.refreshToken },
              "Refresh token error"
            );
            set.status = 401;
      ...
      // TEST MODE ONLY: Auto-login endpoint for E2E tests
      .post(
        "/test-login",
        async ({ body, set, jwt }) => {
